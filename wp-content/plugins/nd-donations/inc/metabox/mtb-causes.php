...
function nd_donations_meta_box()
{
    ...
    $nd_donations_meta_box_subscription = get_post_meta( get_the_ID(), 'nd_donations_meta_box_subscription', true ) 

    ?>


    <p><strong><?php _e('subscription','nd-donations'); ?></strong></p>
    <p><input type="checkbox" name="nd_donations_meta_box_subscription" id="nd_donations_meta_box_subscription" <?php if( $nd_donations_meta_box_subscription == 1 ) { echo 'checked="checked"'; }  ?>  value="<?php echo $nd_donations_meta_box_subscription; ?>" /></p>
...
function nd_donations_meta_box_save( $post_id )
{
    ...
    $nd_donations_meta_box_subscription = sanitize_meta('nd_donations_meta_box_subscription',$_POST['nd_donations_meta_box_subscription'],'post');
    if ( isset( $nd_donations_meta_box_subscription ) ) { 

        if ( $nd_donations_meta_box_subscription != '' ) {
            update_post_meta( $post_id, 'nd_donations_meta_box_subscription' , 1 ); 
        }else{
            delete_post_meta( $post_id,'nd_donations_meta_box_subscription');
        }
